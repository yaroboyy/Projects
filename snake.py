...
# Game loop
game_over = False  # Added game_over variable outside the loop
...
while True:
    ...
    if not game_over:
        ...
        if head_pos[0] == food_pos[0] and head_pos[1] == food_pos[1]:
            score += 1
            food_spawn = False  # Set food_spawn to False after eating the food
        else:
            snake_body.pop()
        ...
        if not food_spawn:
            food_pos = [random.randrange(1, (frame_size_x // square_size)) * square_size,
                        random.randrange(1, (frame_size_y // square_size)) * square_size]
            food_spawn = True  # Set food_spawn to True once food is spawned
    ...
    if game_over:
        show_score(0, red, 'consolas', 50)
        pygame.display.update()
        time.sleep(1)
        init_vars()  # Reset the game after 1 second
